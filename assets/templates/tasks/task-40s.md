...
| max width | 500 px |
| max lines | 3 |
| fallback | `Can you solve it in 40 seconds?` when `description` is null. The headline is the challenge; the ask is a separate line under the ring (§5.4). The first renders opened on "Answer in the comments" as the biggest text on the card — an ask before the viewer knew the question. |
...
| **B** timer | 8 – 1207 | 0.27 – 40.27 | 40.0 s countdown. Accent arc depletes to zero. |
| **C** hold | 1208 – 1252 | — | The ask grows into the payoff (§5.4); statement still readable. Cut. |

**Total: 1253 frames, 41.77 s.**
...
`assets/audio/sfx/tick.wav` at frames **8 + 30k** for k = 0…39 — forty ticks, one per second of
...
```
enter = 8 + round(1200 × u),  u ~ Uniform(0.60, 0.80)     
```
