/**
 * Playwright capture for a story post.
 *
 ...
 *
 * The gate here is the text: if the title or any beat's display line still overflows the card at
 * the minimum font size, the run fails. Text spilling off a card is a defect the viewer sees,
 * and shipping it is worse than not shipping the post.
 */
...
export class StoryTextDoesNotFit extends Error {}

export async function captureStory(run, lesson, opts = {}) {
  const webDist = opts.webDist ?? path.join(CORE, 'web', 'dist');
  if (!fs.existsSync(path.join(webDist, 'story.html'))) {
    throw new Error(`story page not built at ${webDist} — run "npm run web:build"`);
  }
  ...
  const { server, port } = await serve(webDist, 'story.html');
  ...
  try {
    ...
    await page.addInitScript((payload) => { window.__AXI_STORY = payload; }, lesson);
    ...
    if (!fit?.fits) {
      throw new StoryTextDoesNotFit(JSON.stringify(fit?.problems ?? 'the page reported no fit result'));
    }
